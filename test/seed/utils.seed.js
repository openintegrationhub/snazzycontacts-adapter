...
const loginSuccessful = nock('http://canary-api.snazzyapps.de/api/iam/login')
  .post('')
  .reply(200, {
...
const loginFailed = nock('http://canary-api.snazzyapps.de/api/iam/login')
  .post('')
  .reply(400, { error: 'Please enter a valid username and password' });

const loginFailedUser = nock('http://canary-api.snazzyapps.de/api/iam/login')
  .post('')
