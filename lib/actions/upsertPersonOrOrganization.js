...
const { upsertObject, fetchUidFromReference, populateRelations } = require('./../utils/helpers');
...
async function processAction(msg, cfg) {
  ...
  async function emitData() {
    ...
    */
    ...
    let contactObject = msg.body.data;

    const type = contactObject.firstName || contactObject.lastname ? 'person' : 'organization';
    ...
    if (recordUid && recordUid !== '' && recordUid !== 'undefined') {
      ...
      if (cfg.sourceApp) {
        ...
        contactObject = await populateRelations(contactObject, cfg, token);
      } else {
