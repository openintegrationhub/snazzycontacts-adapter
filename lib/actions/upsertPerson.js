...
const {
  upsertObject, fetchUidFromReference, populateRelations, deleteObject,
  ...
  try {
    ...
    */
    ...
    if (cfg.deletes && objectExists && msg.data && msg.data.deleteRequested) {
      // Delete entry
      const reply = await deleteObject(personObject, token, objectExists, 'person', recordUid, cfg.devMode);
      ...
      const reply = await upsertObject(personObject, token, 'person', recordUid, cfg.devMode);
