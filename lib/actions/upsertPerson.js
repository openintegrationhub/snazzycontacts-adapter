...
async function processAction(msg, cfg) {
  ...
  async function emitData() {
    ...
    const { uid } = incomingObject.body;
    console.log('UID: ', uid);

    if (uid) {
      const appObject = await checkForExistingUser(uid, token);
      ...
      if (appObject) {
        ...
        incomingObject = resolved;
      }
    }
    console.log('INCOMING: ', incomingObject);

    const reply = await upsertPerson(incomingObject, token, personExists);
...
async function checkForExistingUser(uid, token) {
  try {
    const options = {
      method: 'GET',
      uri: `${SNAZZY_URI}/${uid}`,
