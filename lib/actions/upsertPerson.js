...
async function upsertPerson(msg, token, personExists) {
  console.log('MSG 1: ', msg.body);
  console.log('PERSON EXISTS: ', personExists);
  if (personExists) {
    console.log('HERE');
    method = 'PUT';
    uri = `${SNAZZY_URI}/${msg.body.rowid}`;
  } else {
    console.log('THERE');
    method = 'POST';
    uri = SNAZZY_URI;
...
function resolveConflict(incomingObject, appObject) {
  ...
  const resolved = cfm.resolve(incomingObject.body.data, appObject.body.data);
