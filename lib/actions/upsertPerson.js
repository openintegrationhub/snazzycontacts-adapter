...
async function processAction(msg, cfg) {
  ...
  async function emitData() {
    ...
    if (msg.body.uid) {
      // TODO: checkForExistingUser only if uid is available
    }

    const checkPerson = await checkForExistingUser(msg, token);
    console.log('CHECK PERSON: ', checkPerson);
...
async function upsertPerson(msg, token, personExists) {
  ...
  if (personExists) {
    ...
    uri = SNAZZY_URI;
    newPerson = msg.body;
    console.log('NEW PERSON BEFORE: ', newPerson);
    delete newPerson.body.uid;
  }
  console.log('NEW PERSON AFTER: ', newPerson);
