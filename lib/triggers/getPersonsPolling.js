...
async function processTrigger(msg, cfg, snapshot = {}) {
  console.log('SNAPSHOT: ', JSON.stringify(snapshot, undefined, 2));
  ...
  console.log(`Last Query: ${snapshot.addQuery}`);
  ...
  async function emitData() {
    ...
    if (persons.length >= 1000) {
      ...
      console.log(`New snapshot: ${JSON.stringify(snapshot, undefined, 2)}`);
      return self.emit('snapshot', snapshot);
    }
    if (persons.length > 0 && persons.length < 1000) {
      persons.forEach((elem) => {
        self.emit('data', messages.newMessageWithBody(elem));
      });
      snapshot.lastUpdated = persons[persons.length - 1].lastUpdate;
