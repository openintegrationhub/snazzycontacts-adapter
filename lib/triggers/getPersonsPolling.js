...
function processTrigger(msg, cfg, snapshot = {}) {
  ...
  async function emitData() {
    const persons = await getPersons(cfg, snapshot);
...
async function getPersons(token, snapshot = {}) {
  try {
    ...
    // const persons = await request.get(requestOptions);
    const persons = await fetchAll(requestOptions, snapshot);
    if (!persons.data || !Array.isArray(persons.data)) throw `Expected records array. Instead received: ${JSON.stringify(persons.data)}`;
    return persons;
  } catch (e) {
    throw new Error(e);
  }
