...
async function processTrigger(msg, cfg, snapshot = {}) {
  ...
  snapshot.lastUpdated = snapshot.lastUpdated || (new Date(0)).getTime(); //(new Date(0)).toISOString(); // Date.now(); or //moment(0).toISOString();
...
async function fetchAll(options, snapshot) {
  try {
    ...
    console.log('BEFORE: ', persons.data[0].firstName, persons.data[0].lastName);
    persons.data.filter((person) => {
      ...
      // console.log('PERSON LAST UPDATE: ', person.lastUpdate, person.firstName);
    ...
    result.sort(function(a, b) {
      ...
      // console.log('A > B: ', a.lastUpdate > b.lastUpdate);
      // console.log(parseInt(a.lastUpdate));
      return parseInt(a.lastUpdate) - parseInt(b.lastUpdate);
    });
    // return persons.data;
    console.log('AFTER: ', result[0].firstName, result[0].lastName);
    return result;
  } catch (e) {
    throw new Error(e);
  }
