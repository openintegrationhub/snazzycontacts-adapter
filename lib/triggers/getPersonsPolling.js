...
async function processTrigger(msg, cfg, snapshot = {}) {
  ...
  async function emitData() {
    ...
    const getCount = await getPersons(token, snapshot, query, count = undefined);
    const { count } = getCount;

    console.log('COUNT: ', count);

    const persons = await getPersons(token, snapshot, query, count);
...
async function getPersons(token, snapshot, query, count) {
  let uri;
  if (count) {
    uri = `http://api.snazzyapps.de/api/person?num=${count}`;
  } else {
