...
async function processTrigger(msg, cfg, snapshot = {}) {
  ...
  snapshot.lastUpdated = snapshot.lastUpdated || new Date('2017-01-01T00:00:00.000Z').toISOString();
  ...
  async function emitData() {
    ...
    if (applicationUidResponse.statusCode === 200 && applicationUidResponse.data) {
    ...
    */
    ...
    if (persons.result.length > 0) {
      persons.result.forEach((elem) => {
        // TODO: Implment 'OPERATION TYPE' for Hub & Spoke
        // if person.updatedAt > person.createdAt ?
        // oihMeta.operation = 'UPDATE' : oihMeta.operation = 'CREATE';
        const newElement = {};
