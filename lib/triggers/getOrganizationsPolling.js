...
const { getEntries, newMessage } = require('./../utils/helpers');
...
async function processTrigger(msg, cfg, snapshot = {}) {
  ...
  // Authenticate and get the token from Snazzy Contacts
    ...
    */
    ...
    const organizations = await getEntries(token, snapshot, 'organization', cfg.devMode);
    ...
    if (organizations.result.length > 0) {
      organizations.result.forEach((elem) => {
        ...
        this.emit('data', newMessage(transformedElement));
      });
      // Get the lastUpdate property from the last object and attach it to snapshot
      snapshot.lastUpdated = organizations.result[organizations.result.length - 1].lastUpdate;
