 ...
 */
async function checkForExistingObject(msg, token, type, devMode) {
  if (!token || !msg) {
    return false;
  }
  ...
  try {
    const options = {
      method: 'GET',
      uri: `${BASE_URI}/${type}/${msg.metadata.recordUid}`,
      json: true,
      headers: {
...
function resolveConflict(incomingObject, appObject) {
  const resolved = cfm.resolve(incomingObject.data, appObject);
  return resolved;
}
