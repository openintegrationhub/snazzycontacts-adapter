...
async function populateRelations(object, cfg, token, objectUid) {
  ...
  for (let i = 0; i < relationObject.relations.length; i += 1) {
    if (!relationObject.relations[i]
      ...
      || relationObject.relations[i].uids.length) relationObject.relations[i].typeRef = '';
    if (objectUid) {
 ...
 * @return {Object} - Array of person objects containing data and meta
 */
async function getEntries(token, snapshot, type, devMode) {
...
module.exports = {
  getEntries, upsertObject, fetchUidFromReference, populateRelations, newMessage,
};
