...
const SNAZZY_URI = 'https://api.snazzycontacts.com/api';
...
async function upsertObject(msg, token, objectExists, type) {
  ...
  if (objectExists) {
    method = 'PUT';
    uri = `${SNAZZY_URI}/${type}/${msg.uid}`;
    newObject = generateObject(msg, type);
  } else {
    method = 'POST';
    uri = `${SNAZZY_URI}/${type}`;
    newObject = msg.body;
    // newObject = generateObject(msg.body, type);
...
function generateObject(msg, type) {
  let newObject;
  if (type === 'person') {
    newObject = {
      dto: {
